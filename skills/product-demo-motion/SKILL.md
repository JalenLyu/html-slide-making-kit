...
```yaml
intent: "prove common PM actions can be run as Codex Skills from Workbench"
candidates:
  - selector: ".jdb-type-tab[href='#jdb-view-skills']"
    role: navigation
    proves: "where the feature lives"
    rank: secondary
  - selector: "#jdb-view-skills .jdb-command-card a[href*='AI']"
    role: primary-action
    proves: "one click triggers a local Skill-backed action"
    rank: primary
selected:
  selector: "#jdb-view-skills .jdb-command-card a[href*='AI']"
  reason: "matches the slide claim better than the Skills navigation tab"
```
...
source:
  ...
  image: Reports/obsidian-workbench-overview.png
  viewport: { width: 2880, height: 1492 }
intent: "show one-click Skill execution from Workbench"
selected_interaction:
  selector: "#jdb-view-skills .jdb-command-card a[href*='AI']"
  reason: "primary action for the slide purpose; Skills tab is only navigation"
scenes:
  - id: overview
    duration: 2.0
  - id: focus-action-panel
    target:
      selector: "#jdb-view-skills .jdb-command-card"
    effect: push-in-blur
  - id: click-run
    target:
      selector: "#jdb-view-skills .jdb-command-actions a[href*='AI']"
      runner: "ai-project-review"
      output: "Projects/project-cleanup-candidates.md"
