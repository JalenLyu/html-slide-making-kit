...
Use this before creating or visually revising HTML slides. It is a compact design library for improving the model's default visual taste without loading a large design system.
...
- Use official local assets from `assets/shopee-design/` when available: logo icon, horizontal logo, ShopeeFont, Campaign KV, and reference screenshots.
...
- Keep the main product or system name clean. Put sample/representative/anonymized status in a separate badge, caption, or corner label rather than appending it to the product name.
