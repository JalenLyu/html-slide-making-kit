...
`beautiful-html-templates` is a MIT-licensed HTML slide template library. It can improve slide taste and layout selection, but it must not override Shopee template constraints when the deck is a Shopee-style business deck.
...
Do not copy the full repository into a slide kit by default; it is about 66 MB and mostly preview screenshots. Store distilled rules here and clone temporarily again when a full external-template deck is needed.
...
## Default Selection For AI Workflow Shares

For internal AI workflow shares, use Shopee template fidelity first, then borrow:
...
`shopee-html-template-real-assets.html` is the local conversion example. It keeps the official Shopee frame while translating selected `beautiful-html-templates` ideas:
